...
class Process(object):
    ...
    @staticmethod
    def create(job_id, script, args, training_status, is_ps=False, config_idx=0):
...
class Job:
    ...
    def set_all_compute_node_status_running(self):
        for compute_node_id, status in self.status_list.iteritems():
    ...
    def get_ps_compute_node_id_list(self):
        ret = []
        for compute_node_id, compute_node_status in self.status_list.iteritems():
    ...
    def get_chief_compute_node_id(self):
        for status in self.status_list.itervalues():
            if status.is_chief:
    ...
    def add_recovery_time_from_chief_worker(self, config_idx, recovery_time):
        ...
        if self.mode == MODE_VALUE_SELFTF:
            self._list_conf_idx_recovery_time[config_idx] += recovery_time
        else:
            "For restart"
            if self._list_conf_idx_recovery_time[config_idx]==.0:
                self.last_recovery_timestamp = time.time()
                self._list_conf_idx_recovery_time[config_idx] += recovery_time
            else:
                self._list_conf_idx_recovery_time[config_idx] += time.time() - \
                                                                 self.last_recovery_timestamp
    ...
    def get_list_history_conf(self):
        return map(lambda x: x[self._configuration_history_config], self.configuration_history)
    ...
    def is_moving_average_smaller_than_target_loss(self):
        ...
        loss_ma = numpy.ma.average(self.list_final_n_machine_loss)
        logging.debug("Job current ma loss: %f" % loss_ma)
...
class PSTunerConfiguration(object):
    ...
    def __str__(self):
        return "num_of_ps:%d, num_of_intra_thread: %d, num_of_partitions: %d, learning_rate: %f, batch_size: %d, " \
               "optimizer: %s" % (self.ps_num,
                                  self.intra_op_parallelism_threads,
                                  self.n_partition,
                                  self.learning_rate,
                                  self.batch_size,
                                  get_optimizer_name(self.optimizer))
...
class PSTunerTrainingData(object):
    def __init__(self, ps_config=None, elapsed_time_in_ms=0, loss=0, step=0, timestamp=time.time(), local_step=0,
                 ps_config_idx=0):
        """
