...
import tuner
import common
from common import PSTunerTrainingDataSerializer, ComputeNode, get_optimizer_name
from message import UpdateNumOfSteps, NaturalFinishML, SendRecoveryTime, \
...
class TFProgramUtil:
    ...
    def __init__(self):
        ...
        self.do_live_reconfig2_context = None
    ...
    def get_intra_op_parallelism_threads(self):
        return self.conf_dict.get(tuner._intra_op_parallelism_threads, 8)

    def get_inter_op_parallelism_threads(self):
        return self.conf_dict.get(tuner._inter_op_parallelism_threads, 8)

    def get_tf_flag(self):
        return self.FLAGS

    def get_tf_config_proto(self, selftf_config_dict=None):
        if selftf_config_dict is None:
            selftf_config_dict = self.conf_dict
        return tf.ConfigProto(
            inter_op_parallelism_threads=selftf_config_dict.get(tuner._inter_op_parallelism_threads, 8),
            intra_op_parallelism_threads=selftf_config_dict.get(tuner._intra_op_parallelism_threads, 8),
    ...
    def post_do_all_iteration(self, sess=None, cost=sys.maxint):
        """
        :param tf.Session sess:
        :return:
        """
        ...
        if self.get_is_chief() and self.flag_do_checkpoint and not self.is_overall_training_finish(last_cost=cost):
            checkpt_time_begin = time.time()
            self.chief_message_handler()
            self.send_recovery_time(self.get_agent_config_idx()+1,
                                    time.time() - checkpt_time_begin)
        # if not self.done_iteration_done:
        #     self.done_iteration_done = True
        #     if self.get_is_chief():
        #         self.logger.debug("I am chief, I will do checkpointing")
        #
        #         # wait a message
        #         with self.conn.Consumer(self.mq_manager.get_chief_queue(), callbacks=[self.chief_message_handler]) as consumer:
    ...
    def pre_do_all_iteration(self, sess):
        ...
        # if it is sync optimizer
        if self._is_sync_optimizer and self.get_is_chief():
            assert(isinstance(self.sync_opt, tf.train.SyncReplicasOptimizer))
            sess.run(self._sync_init_op)

    def term_handler(self, sig, stack):
        self.post_do_all_iteration(self.sess)
        self.conn.close()
        self.stop_set_flag()
        self.mq_thread.join()
    ...
    #
    # def get_variable_task_map(self, graph):
    #     """
    #     :param tf.Graph graph:
    #     :return:
    #     :rtype dict[string, string] :
    #     """
    #     variable_nodes = graph.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES)
    #     return dict(map(lambda variable: (variable.name, variable.device), variable_nodes))
    #
    # class DeviceChooser:
    #
    #     def __init__(self, variable_name_to_job_map, default_chooser_func, _ps_ops, worker_device="/job:worker", total_num_worker=2):
    #         self.variable_name_to_job_map = variable_name_to_job_map
    #         self.default_chooser_func = default_chooser_func
    #         self._ps_ops = _ps_ops
    #         self.total_num_worker = total_num_worker
    #         self.worker_device = worker_device
    #
    #     def choose(self, op):
    #         if self.variable_name_to_job_map[op.name] is not None:
    #             return self.variable_name_to_job_map[op.name]
    #         elif op.op_def.name in self._ps_ops:
    #             return self.worker_device+"/task:%d" % (self.total_num_worker-1)
    #         else:
    #             self.default_chooser_func(op)
    #
    # ps_strategy = [_RoundRobinStrategy,
    #                GreedyLoadBalancingStrategy]
    #
    # def get_ps_strategy(self, ps_num):
    #     ps_strategy_cls = self.ps_strategy[tuner._ps_stragegy]
    #     return self.conf_dict.get(tuner._ps_stragegy, )
    #
    def get_do_common_subexpression_elimination(self):
        return bool(self.conf_dict.get(tuner._do_common_subexpression_elimination, True))

    def get_do_constant_folding(self):
        return bool(self.conf_dict.get(tuner._do_constant_folding, True))

    def get_do_function_inlining(self):
        return bool(self.conf_dict.get(tuner._do_function_inlining, True))

    def get_optimization_level(self):
        return self.conf_dict.get(tuner._inter_op_parallelism_threads, 1)
    ...
    def send_recovery_time(self, conf_idx, recovery_duration):
        ...
        msg = SendRecoveryTime.create(source=self.agent_id,
                                      ...
                                      json_variable_str=json.dumps(tf.trainable_variables(),
    ...
    def chief_message_handler(self, body=None, mq_message=None):
        self.logger.debug("Receive TriggerCheckPointMsg, var_list = %s " % self.saver._var_list)
    ...
    def log_variable_map(self, variable_map=None):
        ...
        for var_name, device in variable_map.iteritems():
    ...
    def build_variable_map(self):
        ...
        logging.debug("Build variable map: %s " % str(ret))
        return ret
    ...
    def do_scheme_2_reconfig(self, new_conf_dict, conf_idx, max_iteration, last_steps):
        ...
        with self.get_default_tf_graph().as_default():
            if self.do_live_reconfig2_context.is_phase_0():
                ...
                # new_optimizer = new_conf_dict.get(tuner._optimizer, "Adam")
                #
                old_variable_map = dict(old_config_dir.get(common.conf_dict_variable_map, {}))
                ...
                #self.do_live_reconfig2_pending_ops = [[opt_init_op]]
                self.do_live_reconfig2_pending_ops = []
                GenericTimer.stop("rebuild_for_modify_graph")
                GenericTimer.print_all_timer()
    ...
    def _init_train_op(self, optimizer, sync=False, sync_exp_moving_averager=None):
        ...
        if sync and self.get_is_chief():
            tf.add_to_collection(tf.GraphKeys.QUEUE_RUNNERS,
                                 optimizer.get_chief_queue_runner())
            self._sync_init_op = optimizer.get_init_tokens_op()

    def init_train_op(self, sync=False, sync_exp_moving_averager=None):
    ...
    def set_train_op(self, loss=None, sync=False, sync_exp_moving_averager=None):
        if len(self._list_variable_name_without_optimizer_variable) == 0:
            self._list_variable_name_without_optimizer_variable = map(
                lambda x: x.op.name,
                tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES))

        # Hack here, prevent optimizer static variable (e.g beta_1_power)
        first_var = min(tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES), key=lambda x: x.name)
    ...
    def get_batch_size(self):
        return self.conf_dict[tuner._batch_size]
    ...
    def get_learning_rate(self):
        return self.conf_dict[tuner._learning_rate]

    def get_n_partition(self):
        return 20
        return self.conf_dict[tuner._n_partition]
    ...
    def get_num_ps(self):
        return self.conf_dict[common._conf_dict_ps_num]
    ...
    def set_graph_init_func(self, func):
        ...
        with self.get_default_tf_graph().as_default():
            self.graph_init_func(self)
            self._list_trainable_variable_name = map(
    ...
    def init_graph(self):
        ...
        self.do_live_reconfig2_context = SelfTFOptimizerContext(
            variable_map = self.create_variable_map(),
            worker_device=self._job_prefix + str(self.get_task_index()),
    ...
    # mock a variable map
    def create_variable_map(self):
        ret = {}
        with tf.get_default_graph().as_default():
            for v in tf.trainable_variables():
                ret[v.op.name] = v.op.device
        return ret
...
class SelfTFOptimizerContext:
    ...
    def __init__(self,
        ...
        self._dict_nested_var_of_tv = self.build_dict_nested_var_of_tv()
    ...
    def _init_wrapper_replica_variables(self):
        list_variable = tf.trainable_variables()
    ...
    def phase1_update_graph(self, variable_map):
        #phase 1
        ...
        for var in list_tv_with_nested_var:
            ...
            logging.debug(
                "replica device: %s var_device:%s op_device: %s" % (
                    device,
                    replica.dst_ps_v.op.device,
                    replica.assign_op_wc_dst_ps.op.device
                ))
    ...
    def init_dict_for_a_graph(self, graph):
        self._graph = graph
        self._dict_nested_var_of_tv = self.build_dict_nested_var_of_tv()
        self._dict_var_ops = self.build_dict_var_ops()
        self._apply_ops = tfge.select_ts(".*/update_.*/.*", graph=self._graph)
    ...
    def get_parent_scope_name(self, v):
        return v.name[:v.name.rfind("/")]

    def get_device_name_by_v(self, search_v):
        """
        ...
        v = self.variable_map.get(v_name)
        if v is None:
            # Maybe a nest var, get by its parent name
            v = self.variable_map[self.get_parent_scope_name(search_v)]
        if v is None:
            raise Exception("can't get the name")
        return v
    ...
    def diff_variable_map(self, new_variable_map, old_variable_map):
        ...
        for var_name in new_variable_map.iterkeys():
    ...
    def update_device_for_optimizer_variables_in_tv(self):
        ...
        :return:
        """
        for tv, list_opt_v in self._dict_nested_var_of_tv.iteritems():
    ...
    def update_input_of_gradient_op(self, wrapper_replica_variables):
        ...
        for replica in wrapper_replica_variables.list_variable_replica:
            ...
            tfge.swap_outputs(self.get_read_ts_from_variable(wc_v),
                              self.get_read_ts_from_variable(v))
    ...
    def get_worker_cache_var(self):
        return map(lambda replica: replica.worker_cache_v,
...
class VariableReplicaWrapper(object):
    ...
    def get_assign_op_to_worker_cache(self):
        return map(lambda a: a.assign_to_worker_cache(),
                   self._list_variable_replica)

    def get_assign_op_to_dst_ps(self):
        return map(lambda a:a.assign_to_dist_ps_v(), self._list_variable_replica)

    def get_assign_ops_dst_ps_to_org_v(self):
        return map(lambda a:a.get_dst_ps_to_org_assign_op(), self._list_variable_replica)
...
class GenericTimer:
    ...
    def print_all_timer(cls):
        ...
        for timer_name, start_end in cls.time_map.iteritems():
