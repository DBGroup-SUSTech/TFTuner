...
home_path = os.getenv("SELFTF_HOME", "/root/pstuner")
...
def random_generating_config(model):
    ...
    list_ps_tuner_config = lhs_runner.get_batch_lhs_config(50, mock_job_obj)
...
def is_diverge(monitor_out_path):
    ...
    m=regex_ma_loss.search(monitor_output)
    if m is not None:
        try:
...
def execute(config):
    ...
    base_cmd = "sh -c 'cd %s; source bin/common.sh;export BATCH_SIZE_RANGE=\"%s\"; " \
               ...
               "export NUM_ITER_PER_OS=%d;sh bin/stop_monitor.sh && sh bin/stop_all_agent.sh " \
    ...
    if model == "SVM":
        cmd = "%s  --target_loss 0.13 " \
              ...
              "--num_Features=29890095 --data_dir=%s'" % (
                                           base_cmd,
                                           python_exe,
                                           "%s/kddb_splits" % dataset_base_path)
    elif model == "LR":
    ...
    def __init__(self,
        ...
        if config is not None:
            ...
            super(BenchmarkConfig, self).__init__(
                learning_rate=config.learning_rate,
                batch_size=config.batch_size,
                num_worker=config.worker_num,
                num_ps = config.ps_num,
...
def get_benchmark_config_from_list(list_in):
    return BenchmarkConfig(model=list_in[0],
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        setting = map(get_benchmark_config_from_list, list_setting)
    else:
